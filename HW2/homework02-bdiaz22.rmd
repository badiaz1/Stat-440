...
- One of the most challenging parts of Homework 01 was trying to be creative in creating a table for the data in the picture. The most challenging parts of these weeks have been trying to bring balance back in my routine. I have been trying to space out my homework more to be more successful. 
...
* The data is within a span of a year. 
* There is a major spike in the beginning of the data.
* At the end of the data show the positivity rate is higher than the daily new cases. 

1. Why is the positivity rate higher than the daily new cases at the end of the data?
2. Why is there a spike in the Univ. Community between August and September?
3. What are the "New Cases" in the data?
...
Months | Chart details 
| --- | ------|
August | Bar graph 
September| line graph
October| blue 
November | gray 
December| orange 
January | Other
February | Positivity % (7 day avg)
March | Fac/Staff
April | Grad
May | Undergrad
June | univ. Community 
...
leg <- fromJSON("https://theunitedstates.io/congress-legislators/legislators-current.json")
names(leg)
str(leg$name)
```
...
library(tidyverse)
library(rvest)
tax <- read.csv("https://raw.github-dev.cs.illinois.edu/stat440-fa21/stat440-fa21-course-content/master/data/rental-inspections-grades-data01.csv?token=AAACSTUJHVYYUZS5OWTMKKLBK6LSK")

prop_url <- paste0("https://champaignil.devnetwedge.com/parcel/view/",tax$Parcel.Number,"/2020")
total_tax <- rep(0, length(prop_url))

for (i in 1:10){
  total_tax[i] =
    html_text(html_nodes(read_html(prop_url[i]),"div~ .inner-label+ .inner-value"),trim=TRUE)
}
...
```{r, }
library(tidyverse)
rental <- read_delim("https://raw.github-dev.cs.illinois.edu/stat440-fa21/stat440-fa21-course-content/master/data/rental-inspections-grades-data02.txt?token=AAACSTR77K2W7KF5SKLKIFLBKVB56", col_types = cols(`Inspection Date` = col_date(format = "%m/%d/%Y"), 
