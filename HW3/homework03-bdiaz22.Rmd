...
```{r}
sheriff <- read.csv("https://raw.github-dev.cs.illinois.edu/stat440-fa21/stat440-fa21-course-content/master/data/ccso-jail-data.dat?token=AAACSTURR5N25I6DJUQNUY3BMZGZA")
...
```{r}
library(tidyverse)
food <- read_csv("https://raw.github-dev.cs.illinois.edu/stat440-fa21/stat440-fa21-course-content/master/data/chicago-food-inspections-data.csv?token=AAACSTRLU67SOJHNC5PUCDLBMY5SC")
...
```{r}
library(data.table)
sheriff$'booking_date' <- as.Date(sheriff$'booking_date', format = "%m/%d/%Y")
sheriff$'released_date'<-as.Date(sheriff$'released_date', format = "%m/%d/%Y")

sheriff$'booking_time' <-lubridate::hms(sheriff$'booking_time')
sheriff$'released_time' <-lubridate::hms(sheriff$'released_time')

str(sheriff)
```

```{r}
food$`Inspection Date`<- lubridate::mdy(food$`Inspection Date`)
...
library(dplyr)
inspections <- food %>%
  filter(food$`Inspection Date` > "2011-09-01")%>%
  ...
  drop_na("AKA Name", "Zip", "Inspection Type", "Results", "Latitude", "Longitude", "Violations", "License #")
...
```{r}
inspectionsF <- inspections%>%
  filter(Results== "Fail")

count_inspect  = sort(table(inspectionsF$`Facility Type`))
...
-The third most common facility type is Schools. 
...
```{r, eval = FALSE}
inmates <- sheriff%>%
  mutate(booking_date_time = ymd_hms(paste(booking_date, booking_time)))%>%
  mutate(released_date_time =  ymd_hms(paste(released_date, released_time)))%>%
  mutate(years = as.integer(floor(difftime(released_date_time, booking_date_time, units = "days")/365)))%>%
  mutate(days = as.integer(floor(difftime(released_date_time, booking_date_time, units = "days")%%365))) %>%
  mutate(hours2 = as.integer(floor(difftime(released_date_time, booking_date_time, units = "hours") %% 24))) %>%
  mutate(minutes = as.integer(floor(difftime(released_date_time, booking_date_time, units = "mins")%%60))) %>%
  mutate(seconds = as.integer(floor(difftime(released_date_time, booking_date_time, units = "secs")%%60))) %>%
  mutate(time_spent_in_jail1 = as.numeric(365*24*years + 24*days + hours2 + minutes/60 + seconds/3600)) %>%
  mutate(time_spent_in_jail2 = as.numeric(difftime(released_date_time, booking_date_time, units="hours")))%>%
  rename(
    booking_number = booked_number
    booking_date =  booked_date
    booking_time = booked_time
    booking_reason = booked_reason
)


head(select(inmates, c("jacket_number", "time_spent_in_jail1", "time_spent_in_jail2")),  10)

#I am having a hard time trying to figure out how to fix the error to this code. 
